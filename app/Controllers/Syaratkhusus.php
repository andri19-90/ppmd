...
class Syaratkhusus extends BaseController {
    ...
    public function index(){
        if(session()->get("logged_in")){
            ...
            $cek = $this->model->getAllQR("SELECT count(*) as jml FROM syarat_khusus;")->jml;
            if($cek > 0){
    ...
    private function simpan() {
        $data = array(
            'idsyarat_khusus' => $this->model->autokode("K","idsyarat_khusus","syarat_khusus", 2, 7),
            'keterangan' => $this->request->getPost('ket')
        );
        $simpan = $this->model->add("syarat_khusus",$data);
        if($simpan == 1){
    ...
    private function update() {
        ...
        $update = $this->model->updateNK("syarat_khusus", $data);
        if($update == 1){
